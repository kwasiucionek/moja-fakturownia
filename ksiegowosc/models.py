...
KODY_URZEDOW_SKARBOWYCH = [
    ...
    ("3028", "Urząd Skarbowy we Wrześni"),
...
class Invoice(models.Model):
    ...
    ksef_status = models.CharField(
        ...
        default="draft",
        verbose_name="Status KSeF",
    )
    ksef_reference_number = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        verbose_name="Numer referencyjny dokumentu",
    )
    ksef_session_id = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="ID Sesji KSeF"
    )
    ksef_number = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Numer KSeF (z UPO)"
    )
    ksef_sent_at = models.DateTimeField(
        blank=True, null=True, verbose_name="Data wysyłki"
    )
...
class ZUSRates(models.Model):
    ...
    def calculate_social_insurance(self, company_info, annual_income=None):
        if company_info.preferential_zus:
            base = self.minimum_wage * Decimal("0.60")
        elif (
            ...
            base = max(
                self.minimum_base * Decimal("0.30"),
                min(annual_income * Decimal("0.50") / 12, self.minimum_base),
            )
        else:
            base = self.minimum_base

        pension_disability = base * (self.pension_rate + self.disability_rate)
        accident = base * self.accident_rate
